...
export default function Login() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Demo auth: accept any password of length >= 8, otherwise show incorrect.
    if (password.length < 8) {
      setErrors(prev => ({ ...prev, auth: "Password is incorrect" }));
      return;
    }

    // On success, navigate to dashboard/success (demo)
    navigate("/success");
  };
  ...
  <div className="w-full max-w-[400px]">
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-[32px] font-bold leading-normal text-[#2E3139]">Welcome Back</h1>
        <p className="text-sm leading-normal text-[#425583]">Sign in to your account</p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <input
          ...
          placeholder="Email: your@example.com"
          className={`w-full h-12 px-4 rounded-full border ${
            errors.email ? "border-red-500" : "border-[#D3E0FE]"
      ...
      <div className="flex flex-col gap-4">
        <div className="relative">
          <input
            ...
            // ADDED placeholder for Password
            placeholder="Password: ••••••••"
            className={`w-full h-12 px-4 pr-12 rounded-full border ${
              errors.password || errors.auth ? "border-red-500" : "border-[#D3E0FE]"
            } focus:border-[#4045EF] focus:outline-none transition-colors`}
          />
          <button
            type="button"
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 text-sm text-[#425583]">
                <input type="checkbox" checked={remember} onChange={(e) => setRemember(e.target.checked)} />
                Remember me
              </label>
              <Link to="/forgot-password" className="text-sm text-[#4045EF] hover:underline">Forgot Password</Link>
            </div>

            <button type="submit" className="h-12 w-full rounded-full bg-gradient-to-r from-[#5B53FF] to-[#4045EF] text-white font-medium">Sign in</button>

            <p className="text-center text-sm text-[#425583]">Don't have an account? <Link to="/" className="text-[#4045EF] font-medium">Sign up</Link></p>
          </form>
        </div>
      </div>
    </div>
